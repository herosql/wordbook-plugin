...
submitBtn.addEventListener('click', function () {
    ...
    if (!lang) { confirmText.textContent = '请选择语言'; confirmYes.textContent = '确定'; confirmNo.style.display = 'none'; modal.classList.add('show'); confirmYes.onclick = function () { modal.classList.remove('show'); confirmNo.style.display = ''; }; return; }
...
function doSubmit(words, lang) {
    ...
    chrome.cookies.getAll({ url: DEFAULT_URL }, function (cookies) {
        ...
        words.forEach(function (word) {
            var targetUrl = 'https://dict.youdao.com/wordbook/webapi/v2/ajax/add?word=' + encodeURIComponent(word.trim()) + '&lan=' + lang;
            fetch(targetUrl, {
                method: 'GET',
